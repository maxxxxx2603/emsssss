...
async def semaine(interaction: discord.Interaction):
    ...
    if log_channel and pre_stats:
        ...
        # Ajouter les heures de service au bilan
        ...
        if week_svc:
            ...
            h_t = int(total_h)
            m_t = int((total_h - h_t) * 60)
            svc_summary += f"\n**Total heures :** {h_t}h{m_t:02d}"
        
        summary_embed = discord.Embed(
            title="📊 RÉSUMÉ SEMAINE EMS",
            description=f"**Total des réanimations (semaine):** `{total_reactions}` 🎯{svc_summary}",
            color=EMS_RED
        )
        summary_embed.set_footer(text="🚑 EMS System")
        embeds.append(summary_embed)
...
async def leaderboard_command(interaction: discord.Interaction):
    ...
    # Ajouter les heures de service du top 5
    services = load_services()
    week = get_week_start()
    week_services = services.get(week, {})
    if week_services:
        ...
        for key, value in top_5:
            normalized = normalize_employee_key(key)
            svc = week_services.get(key) or week_services.get(normalized)
            if svc:
                h = int(svc['total_hours'])
                m = int((svc['total_hours'] - h) * 60)
                display = key.replace('-', ' ').title()
                hours_text += f"• **{display}** : {h}h{m:02d}\n"
        if hours_text:
            embed.add_field(name="⏱️ Heures de service", value=hours_text, inline=False)
