...
# --- CONFIGURATION ---
# Priorité : volume Railway (/data/config.json) > local (config.json) > variables d'environnement
_DATA_DIR_EARLY = os.environ.get("RAILWAY_VOLUME_MOUNT_PATH", ".")
_CONFIG_PATHS = [
    os.path.join(_DATA_DIR_EARLY, 'config.json'),
    'config.json',
]
config = None
...
# Configuration Taxi
TAXI_CHANNEL_ID = cfg("TAXI_CHANNEL_ID", 1457304629456011264)
TAXI_ROLE_ID = cfg("TAXI_ROLE_ID", 1163206112355561472)
ROLE_DIRECTION_EMS_ID = cfg("ROLE_DIRECTION_EMS_ID", 838120186585940010)
ROLE_DIRECTION_TAXI_ID = cfg("ROLE_DIRECTION_TAXI_ID", 1311787019546136596)

# Configuration BurgerShot
BURGERSHOT_CHANNEL_ID = cfg("BURGERSHOT_CHANNEL_ID", 1462099226166165588)
BURGERSHOT_ROLE_ID = cfg("BURGERSHOT_ROLE_ID", 1462097148995965041)
BURGERSHOT_STATS_FILE = os.path.join(DATA_DIR, 'burgershot_stats.json')

# Configuration Tickets
ROLE_REQUEST_CHANNEL_ID = cfg("ROLE_REQUEST_CHANNEL_ID", 1450938023033176247)
APPOINTMENT_CHANNEL_ID = cfg("APPOINTMENT_CHANNEL_ID", 1415783172163244132)
TICKET_CATEGORY_ID = cfg("TICKET_CATEGORY_ID", 840364236189335553)
ROLE_LSPD_ID = cfg("ROLE_LSPD_ID", 1070687458825601115)
ROLE_BCSO_ID = cfg("ROLE_BCSO_ID", 1070374792450027560)
ROLE_MARSHALL_ID = cfg("ROLE_MARSHALL_ID", 1365068483074855045)
ROLE_NO_TEST_ID = cfg("ROLE_NO_TEST_ID", 1163524216688230591)
ROLE_TAXI_REQUEST_ID = cfg("ROLE_TAXI_REQUEST_ID", 1311784189984505876)

# Configuration Reset
ROLE_BASE_ID = 838102445095256066  # Rôle de base à conserver
RESET_CHANNEL_ID = cfg("RESET_CHANNEL_ID", 1450938023033176247)

# Configuration Leaderboard
LEADERBOARD_CHANNEL_ID = cfg("LEADERBOARD_CHANNEL_ID", 1018904202971459644)
LEADERBOARD_ROLE_ID = cfg("LEADERBOARD_ROLE_ID", 838102445095256068)

# Configuration Avis
AVIS_CHANNEL_ID = cfg("AVIS_CHANNEL_ID", 1478910608228487255)
CITOYEN_ROLE_ID = cfg("CITOYEN_ROLE_ID", 838102445095256066)

# Configuration Dispo
DISPO_REQUEST_CHANNEL_ID = cfg("DISPO_REQUEST_CHANNEL_ID", 1478916243858915591)  # Canal pour les demandes initiales
DISPO_CHANNEL_ID = cfg("DISPO_CHANNEL_ID", 1478912686069780602)  # Canal pour les décisions de recrutement
DISPO_CONFIRMATION_ROLE_ID = cfg("DISPO_CONFIRMATION_ROLE_ID", 896103247096471613)
DIRECTION_ROLE_ID = cfg("DIRECTION_ROLE_ID", 838120186585940010)  # Rôle direction pour validation dispo et recrutement
ROLE_PENDING_ID = cfg("DISPO_CONFIRMATION_ROLE_ID", 896103247096471613)
...
# ── Roles config & options dynamiques (depuis /options sur le dashboard) ──
_DEFAULT_ROLES_CONFIG = {
    "grades": [
        {"tag": "DIR", "label": "Directeur Médical", "role_id": 1088570974603055195, "category_id": 0, "salary_rate": 55000, "order": 1, "enabled": True},
        {"tag": "CAD", "label": "Chef Adjoint",       "role_id": 1528561040663777310, "category_id": 1528562915458814053, "salary_rate": 52000, "order": 2, "enabled": True},
        {"tag": "CDS", "label": "Chef de Service",    "role_id": 838102445095256071,  "category_id": 0, "salary_rate": 50000, "order": 3, "enabled": True},
        {"tag": "MED", "label": "Médecin",            "role_id": 840288242547818507,  "category_id": 0, "salary_rate": 45000, "order": 4, "enabled": True},
        {"tag": "PSY", "label": "Psychologue",        "role_id": 1528560704511148092, "category_id": 1528562582804365312, "salary_rate": 42000, "order": 5, "enabled": True},
        {"tag": "INF", "label": "Infirmier",          "role_id": 894311352225656862,  "category_id": 0, "salary_rate": 40000, "order": 6, "enabled": True},
        {"tag": "ADS", "label": "Aide-Soignant",      "role_id": 1088116715998687273, "category_id": 0, "salary_rate": 40000, "order": 7, "enabled": True},
        {"tag": "STG", "label": "Stagiaire",          "role_id": 838102445095256069,  "category_id": 1515101531186397335, "salary_rate": 35000, "order": 8, "enabled": True},
        {"tag": "EMT", "label": "EMT",                "role_id": 895047492784238652,  "category_id": 1515101532746678332, "salary_rate": 30000, "order": 9, "enabled": True},
    ],
    "category_indispo_id": 1524182164336279682,
    "direction_salary": 9000000,
    "direction_role_id": 838120186585940010,
    "extra_roles_to_remove": [838102445095256068, 838102445095256070],
}

def load_roles_config() -> dict:
    """Charge la config des grades depuis roles_config.json (éditable via /options)."""
    cfg = robust_load_json(ROLES_CONFIG_FILE, {})
    if not cfg or 'grades' not in cfg:
        return _DEFAULT_ROLES_CONFIG
    return cfg

def load_options() -> dict:
    """Charge les options du bot depuis options.json (éditable via /options)."""
    default = {
        "leaderboard": True, "dispo": True, "giveaway": True, "burgershot": True,
        "tickets": True, "reunions": True, "formations": True, "parrainage": True,
        "patients": True, "avertissements": True, "absences": True, "cv_system": True,
        "dispatch": True, "matricules": True, "badges": True, "semaine_auto": True,
    }
    opts = robust_load_json(OPTIONS_FILE, {})
    default.update(opts)
    return default
...
def get_grade_salary_rate(grade_tag: str) -> int:
    ...
    for g in cfg.get('grades', []):
        if g.get('tag', '').upper() == grade_tag.upper() and g.get('enabled', True):
            return int(g.get('salary_rate', 0))
    return 0

def get_grade_role_id(grade_tag: str) -> int:
    """Retourne le Discord Role ID pour un grade."""
    cfg = load_roles_config()
    for g in cfg.get('grades', []):
...
def get_grade_to_category() -> dict:
    ...
    for g in cfg.get('grades', []):
        tag = g.get('tag', '').upper()
        cat_id = int(g.get('category_id', 0))
        if tag and cat_id:
            result[tag] = cat_id  # IDs directs plutôt que clés
    return result

# Fonction pour charger les catégories
def load_categories():
    """Charge les catégories depuis roles_config.json (priorité) puis categories.json (rétrocompat)."""
    # Priorité : roles_config (éditable via /options)
    roles_cfg = load_roles_config()
    from_roles = {}
    for g in roles_cfg.get('grades', []):
        tag = g.get('tag', '').upper()
        cat_id = int(g.get('category_id', 0))
        key = f"CATEGORY_{tag}_ID"
        from_roles[key] = cat_id
    # Fallback legacy
    legacy_default = {
        "CATEGORY_EMT_ID": 1515101532746678332,
        "CATEGORY_STG_ID": 1515101531186397335,
        "CATEGORY_ADS_ID": 0,
        "CATEGORY_INF_ID": 0,
        "CATEGORY_PSY_ID": 1528562582804365312,
        "CATEGORY_MED_ID": 0,
        "CATEGORY_CDS_ID": 0,
        "CATEGORY_CAD_ID": 1528562915458814053,
        "CATEGORY_DIR_ID": 0
    }
    legacy = robust_load_json(CATEGORIES_FILE, legacy_default)
    # Merge: roles_config a priorité si non nul
    merged = dict(legacy)
    for k, v in from_roles.items():
        if v:  # roles_config prime si valeur non nulle
            merged[k] = v
    return merged
...
class CVButton(discord.ui.View):
    ...
    async def start_cv(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if cv_channel:
            ...
            print(f"❌ Channel CV non trouvé (ID: 1539697901540745317)")
    ...
    async def start_formulaire(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            if cv_channel:
                ...
                print(f"❌ FormulaireCVButton: Channel CV non trouvé (ID: 1539697901540745317)")
...
async def reset_names(interaction: discord.Interaction, membre: discord.Member = None):
    ...
    role_hierarchy = get_role_hierarchy() or [
        (1088570974603055195, "[DIR]"),
        (1528561040663777310, "[CAD]"),
        (838102445095256071, "[CDS]"),
        (840288242547818507, "[MED]"),
...
async def executer_virement(interaction: discord.Interaction, membre: discord.Member, raison_type: str, raison_custom: str = None):
    ...
    _dynamic_ems_ids = get_all_ems_role_ids() or EMS_ROLE_IDS_TO_REMOVE
    roles_to_remove = [r for r in membre.roles if r.id in _dynamic_ems_ids]
...
async def payes(interaction: discord.Interaction):
    ...
    _dir_role_id = int(load_roles_config().get('direction_role_id', ROLE_DIRECTION_EMS_ID))
    direction_role = guild.get_role(_dir_role_id) or guild.get_role(ROLE_DIRECTION_EMS_ID)
    ...
    for member in guild.members:
        ...
        if has_direction_role:
            # DIRECTION: salaire fixe (éditable via /options)
            clean_name = get_clean_name(member)
            salary = int(load_roles_config().get('direction_salary', 9000000))
        ...
        # Détection dynamique depuis roles_config (éditable via /options)
        _dyn_cfg = load_roles_config()
        _known_tags = [g['tag'].upper() for g in _dyn_cfg.get('grades', []) if g.get('enabled', True)]
        for _tag in _known_tags:
            if f"[{_tag}]" in nick or (_tag == "STG" and "[INT]" in nick):
                grade = _tag
                rate = get_grade_salary_rate(_tag)
                break
...
CATEGORY_INDISPO_ID = load_roles_config().get('category_indispo_id', 1524182164336279682)
...
async def reouverture(interaction: discord.Interaction, membre: discord.Member):
    ...
    # Récupérer la bonne catégorie selon le grade (dynamique depuis roles_config)
    _grade_cats = get_grade_to_category()
    cat_id = _grade_cats.get(grade_found, 0)
    if not cat_id:  # Fallback legacy
...
async def remise(interaction: discord.Interaction):
    """Remet à jour TOUS les employés avec le rôle 838102445095256068, quelque soit leur catégorie"""
    await interaction.response.defer(ephemeral=True)
    ...
    try:
        ...
        # Obtenir tous les membres avec le rôle employé (TOUS, partout)
        employes_discord = [m for m in guild.members if role_employe in m.roles]
        print(f"[REMISE] Mise à jour de {len(employes_discord)} employés")
        
        # Collecter tous les channels de TOUTES les catégories + niveau racine
        ...
        # Mettre à jour chaque employé (peu importe sa catégorie)
        updated_count = 0
        failed_channels = []
        ...
        for member in employes_discord:
            try:
                ...
                if found_channel:
                    # Déterminer le total de réas
                    total_reas = 0
                    
                    # Chercher dans _REMISE_STATS (clés en kebab-case)
                ...
                failed_channels.append((member.display_name, "Permission refusée"))
            except Exception as e:
                print(f"[REMISE] ❌ Erreur pour {member.name}: {e}")
                failed_channels.append((member.display_name, str(e)))
        
        # Créer l'embed récapitulatif
        embed = discord.Embed(
            title="🟢 REMISE À JOUR COMPLÈTE",
            description=f"**{updated_count}/{len(employes_discord)}** employés mis à jour",
            color=discord.Color.green()
        )
        
        embed.add_field(
            name="✅ Mises à jour appliquées",
            value=f"• 📝 Format: `🔴username`\n• 📌 Description: `🔴 Name • Total/100 réas`\n• 🎯 Tous les employés du rôle traités\n• 🔴 Sans réas = rouge",
            inline=False
        )
        
        if failed_channels:
            failed_text = "\n".join([f"• {name}: {reason}" for name, reason in failed_channels[:15]])
            embed.add_field(name="⚠️ Non trouvés/Erreurs", value=failed_text, inline=False)
...
async def sync_channels(interaction: discord.Interaction, dry_run: bool = True):
    ...
    _ems_sync_raw = cfg("EMS_SYNC_ROLE_ID", 0)
    EMS_ROLE_ID = _ems_sync_raw if _ems_sync_raw else 838102445095256068
